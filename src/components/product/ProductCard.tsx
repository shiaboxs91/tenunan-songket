...
export function ProductCard({ product, density = "comfortable", onQuickView }: ProductCardProps) {
  ...
  return (
    <Card className="group overflow-hidden h-full hover:shadow-lg transition-shadow duration-300 relative border-amber-200/50 hover:border-amber-300/70">
      ...
      >
        <span className="sr-only">Lihat detail {product.title}</span>
      </Link>

      {/* Elegant corner ornaments */}
      <div className="absolute top-0 left-0 z-10 opacity-60 group-hover:opacity-100 transition-opacity pointer-events-none">
        <CornerOrnament />
      </div>
      <div className="absolute top-0 right-0 z-10 opacity-60 group-hover:opacity-100 transition-opacity rotate-90 pointer-events-none">
        <CornerOrnament />
      </div>
      <div className="absolute bottom-0 left-0 z-10 opacity-60 group-hover:opacity-100 transition-opacity -rotate-90 pointer-events-none">
        <CornerOrnament />
      </div>
      <div className="absolute bottom-0 right-0 z-10 opacity-60 group-hover:opacity-100 transition-opacity rotate-180 pointer-events-none">
        <CornerOrnament />
      </div>
      ...
      <CardContent className={cn(isCompact ? "p-2" : "p-3", "relative z-10 pointer-events-none")}>
        {/* Category */}
        <p className={cn(
